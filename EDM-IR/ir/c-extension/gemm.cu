...
template <typename T, int KTileM, int KtileK, typename TiledMMA>
__globel__ void gemm_simple(T *Cptr, const T*Aptr, const T*Bptr, int m, int n,int k)
...
auto tAgA = thr_mma.partition_A(g) auto tBgB = thr_mma.partition_B(gB);  // (MMA, MMA_N, MMA_K, num_tile_k)
